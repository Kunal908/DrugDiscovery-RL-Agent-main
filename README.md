...
- [Actions the Agent Can Take](#-actions-the-agent-can-take)
- [How to Use](#️-how-to-use)
- [Saved Model Paths](#-saved-model-paths)
...
- `drug_env.py`: Gym-compatible environment for molecular editing using SMILES strings
- `agent_crl.py`: Classical neural network policy agent implemented in PyTorch
- `agent_qrl.py`: Quantum reinforcement learning agent using PennyLane quantum circuits
- `agent_qiskit.py`: Quantum agent simulated via Qiskit Aer
- `agent_ibmq.py`: Agent for real IBM Quantum hardware execution
- `train_compare.py`: Main script for training and evaluating agents
- `models/`: Directory containing saved PyTorch model weights (.pth files)
- `tests/`: Unit tests for all components using pytest
...
```bash
git clone https://github.com/Kunal908/DrugDiscovery-RL-Agent-main.git
cd DrugDiscovery-RL-Agent-main
```
